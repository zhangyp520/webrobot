...
//鼠标事件处理
function CSSViewerMouseDown(event){
	var btnNum = event.button;
	if (btnNum==2)
	{
		//鼠标右键 右键操作时禁用相关操作，后准备选择
		cssViewer.Disable();
	}
	else if(btnNum==0)
	{
...
function CSSViewer()
{
	...
	this.RemoveEventListeners = function()
	{
		...
		for (var i = 0; i < elements.length; i++){
			elements[i].removeEventListener("mousedown", CSSViewerMouseDown, false);
...
* Disable CSSViewer
*/
CSSViewer.prototype.Disable = function(callback)
{
	var document = GetCurrentDocument();
	var block = document.getElementById('CSSViewer_block');
...
{   
	if( 'el' == type ){
		...
		window.parent.postMessage({
			cmd:'cmd4pickSelector',//加载系统和系统菜单
			param:{
				selector:selector
			}
		},'*');//跨域处理
