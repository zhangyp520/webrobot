!(function (win, $) {
    ...
    var PortalController = {
        ...
        init:function(){
            ...
            if(me.data.helpState =="edit" || me.data.helpState =="runtime4preview" ){ //非运行态不执行 帮助向导
                if(me.data.helpState =="runtime4preview" ){
                    //var url = me.data.webAddress+'/help-components/index.js';
                    var url = chrome.extension.getURL("help-components/index.js");
        ...
        event2showLayoutPanel:function(cfg){
            ...
            var $helpPlugin = $(cfg.el).parent();
            if($helpPlugin.hasClass('help-plugin-show')){
                me.hidePlugin();
            }else{
                me.showPlugin();
            }
        ...
        event2toggle:function(cfg){
            var me = this;
            var $sysPanel = $('#help-plugin .help-sys');
            var $eidt = $('#help-plugin .help-page-edit');
            if($sysPanel.hasClass('current')){
                ...
                var $iframe = $('#help-plugin-iframe');
                if(!$iframe[0].src){
                    $iframe[0].src = me.data.helpPluginUrl;
                }
            }
        },
        //显示插件
        showPlugin:function(){
            $('#help-plugin').addClass('help-plugin-show');
            var me = this;
        ...
        changeIframeHeight:function(){
            ...
            if($iframe && $iframe.length){
                ...
                var height = $('#help-plugin .help-plugin-view.current').height();
                $iframe[0].height=height-2;
            }
        },
        //隐藏插件
        hidePlugin:function(){
            $('#help-plugin').removeClass('help-plugin-show');
        },
        _data:function(){
          return this;
        },
            ...
            window.addEventListener("message", function(event) {
                ...
                if(typeof data =='object'){
                    ...
                    if(data.cmd&&data.param){
                        ...
                        console.log('help-plugin:'+data.cmd);
                        console.log(data);
        ...
        renderByLoadSys:function(hasChange){
            oui.getById('help-page-edit-content').render(); //编辑页面渲染
            oui.getById('help-sys-header-info').render(); //系统信息头部渲染
            if(hasChange){
                oui.router.push('tpl/welcome.vue.html');
            }
        },
