...
(function (win, oui, $) {
    ...
    TableGrid.tableRowsTpl[0] =
        '{{if noNeedFirstTr +\'\' != \'true\'}}' + TableGrid.firstTrTpl + '{{/if}}' +
        ...
        '                {{=td.text}}' +
        '                {{else}}' + TableGrid.tableCheckbox4TrTpl + '{{/if}}' +
        '            </div>' +
        '        </td>' +
        '        {{else if td.columnType == "indexcolumn"}}' +
        '        <td id="{{ouiId}}cell_{{td._index}}" class="oui-table-grid-cell {{td.cls}}" style="{{td.cellStyle}}">' +
    ...
    TableGrid.templateHtml[1] = '{{if styleHtml && styleHtml.length > 0}}<style type="text/css">{{styleHtml}}</style>{{/if}}' +
    ...
    var configSetterGetter4Attr = function () {
        ...
        self["getData"] = function (isEdit) {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                var newObj = {};
                oldObj = data[i];
                if (typeof isEdit === 'undefined' || !isEdit) {//如果不是获取可编辑的则将全部字段继承到新的对象中
                    newObj = $.extend(true, {}, rowObjectFilter(oldObj));
                }
                for (var j = 0, jLen = editColumnsArray.length; j < jLen; j++) {//将可编辑的字段，都编程value display 的对象
                    var fieldName = editColumnsArray[j]["fieldName"];
                    var fieldValue = oui.JsonPathUtil.getJsonByPath(fieldName, oldObj);
                    if (fieldValue) {
                        if (typeof fieldValue !== "object") {
                            oui.JsonPathUtil.setObjByPath(fieldName, newObj, {display: "", data4DB: "", value: fieldValue});
                        } else {
                            oui.JsonPathUtil.setObjByPath(fieldName, newObj, fieldValue );
                        }

                    } else {
                        oui.JsonPathUtil.setObjByPath(fieldName, newObj, {display: "", data4DB: "", value: ""});
                    }
                }
                ...
                for (var j = 0, jLen = editColumnsArray.length; j < jLen; j++) {//将可编辑的字段，都编程value display 的对象
                    ...
                    if (typeof fieldValue !== "undefined" && fieldValue !== null) {
                        if (typeof fieldValue !== "object") {
                            ...
                            oui.JsonPathUtil.setObjByPath(fieldName, newObj, (fieldValue.value + ''));
                        }
                    }
                }
                newData.push(newObj);
        ...
        for (var key in newRowData) {
            ...
            if ((rowDataFieldData && ('object' === typeof rowDataFieldData))) {
                ...
                for (var fieldKey in rowDataFieldData) {
                    if (fieldKey + '' === 'display' || fieldKey + '' === 'data4DB' || fieldKey + '' === 'value') {
    ...
    var _parseColumn = function () {
        ...
        if (columns && columns.length > 0) {
            ...
            _parseColumnsPutArrayAndMap(columnsArray, columnsFieldsArray, columnsMap, columns, editColumnsArray, edit4ReadOnlyColumnsArray);
     ...
     * @private
     */
    var _parseColumnsByTag = function (columns, columns$Dom, level, _specialColumns) {
            ...
            $children.each(function () {
                ...
                if ($child.is("oui-column")) {
                    if ($child.find("oui-columns").length > 0) {//如果含有下子列配置
                        ...
                        //递归获取下一子列配置
                        column.columns = [];
                        _parseColumnsByTag(column.columns, $child.children(), level + 1, _specialColumns);
                    } else {
                ...
                column.header = formatNumber(column['headerFormat'], column.header, column['headerDotNum']);
                columns.push(column);
            });
        }
        return level;
        ...
        for (var i = 0, len = columns.length; i < len; i++) {
            ...
            for (var key in defaultColumnAttrs) {
                if (key === 'align' || key === 'headerAlign') {
                    ...
                    _column[key] = typeof _column[key] === 'undefined' ? defaultColumnAttrs[key] : _column[key];
                }
            }

            if (_columns && _columns.length > 0) {
                level = level + 1;
                var obj = _parseColumnsIdByArray(index, id, _column["_id"], _columns, level);
                id = obj.id;
                index = obj.index;
            } else {
     ...
     * @private
     */
    var _parseColumnsPutArrayAndMap = function (columnsArray, columnsFieldsArray, columnsMap, columns, editColumnsArray, edit4ReadOnlyColumnsArray) {
        for (var i = 0, len = columns.length; i < len; i++) {
            var _column = columns[i];
            ...
            if (_column['fieldName'] && _column['fieldName'].length > 0) {
                ...
                if (_column.$ouiControlDom) {
                    ...
                    if (!right || right === 'edit' || right === 'edit4ReadOnly' || right === 'edit4View') {//编辑不可改也属于编辑
            ...
            if (_columns && _columns.length > 0) {
                _parseColumnsPutArrayAndMap(columnsArray, columnsFieldsArray, columnsMap, _columns, editColumnsArray, edit4ReadOnlyColumnsArray);
            }
        }
    };
    ...
    var replaceFiledName = function (calcFormula) {
        ...
        if (fields && fields.length > 0) {
            for (var i = 0, len = fields.length; i < len; i++) {
                ...
                calcFormula = calcFormula.replace(field, "this.getValueByFieldName('" + fieldName + "')");
    ...
    var serverData2tableData = function (data, columnFields, oldDataLength) {
        ...
        if (sortCfg && sortCfg.sortField && sortCfg.sortField.length > 0) {
            ...
            data = data.sort(function (a, b) {
                ...
                return typeof a < typeof b ? (sortOrder === "asc" ? 1 : -1) : (sortOrder === "asc" ? -1 : 1);
            });
        }
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            for (var j = 0, jlen = columnFields.length; j < jlen; j++) {
                ...
                tdData["cellStyle"] = oui.styleExtend(cellStyle4Table, columnField['cellStyle']);
                ...
                if (!(columnField['columns'] && columnField['columns'].length > 0)) {
                    ...
                    if (columnField.columnType && columnField.columnType.length > 0) {//如果是特殊列
                        tdData["columnType"] = columnField.columnType;
                        var sText = null, _stext;
                        if (columnRenderFunc) { //如果有渲染函数
                            if (typeof columnRenderFunc === 'function') {
                                ...
                                try {
                                    ...
                                    _stext = f(null, item, item["_index"], columnField, tableOuiId);
                                } catch (e) {
                        ...
                        try {
                            ...
                            if (calcStr && calcStr.length > 0) {
                                ...
                                var calcFunc = new Function("formatType", "item", "dotNum", calcStr);
                                text = calcFunc.call(calcObj, formatType, item, columnField['formatDotNum']);
                                if (text + '' === 'Infinity') {
                                    text = 0;
                                }
                            }
                        ...
                        if (!$ouiControlDom) {
                            if (columnRenderFunc) { //如果有渲染函数
                                if (typeof columnRenderFunc === 'function') {
                                    ...
                                    try {
                                        var f = evalFunc(columnRenderFunc);
                                        _text = f(text, item, item["_index"], columnField, tableOuiId);
                                    } catch (e) {
                                        throw new Error(columnRenderFunc + " 执行异常");
                                    }
                                }
                            ...
                            tdData["text"] = $.trim(text).length > 0 ? ((isEscape + "" === "true") ? oui.escapeStringToHTML(text, false) : text) : "&nbsp;";
                        } else {
                            text = (text && (typeof text.value !== "undefined")) ? text : {
                                value: text,
                                data: ""
                            };
                            title = text.value || '';
                            if (editModel !== 'all') {
                                ...
                                title = textDisplay;
                                textDisplay = $.trim(textDisplay).length > 0 ? ((isEscape + "" === "true") ? oui.escapeStringToHTML(textDisplay, false) : textDisplay) : "&nbsp;";
                                tdData["text"] = textDisplay;
                                tdData["isEdited"] = !!text.isEdited;
                            } else {
                                var ouiFormId = self.attr("id") + "rows" + item["_index"] + "cell" + tdData["_index"] + "_control";
                                var _$ouiControlDom = $($ouiControlDom).clone().attr("id", ouiFormId);
                                var onUpdateStr = _$ouiControlDom.attr("onupdate");
                                var formUpdate = "oui.getByOuiId(" + self.attr("ouiId") + ").formControlUpdate";
                                ...
                                var formControlUpdateAfter = "oui.getByOuiId(" + self.attr("ouiId") + ").formControlUpdateAfter";
                                ...
                                if (onOuiFormClone) {
                                    ...
                                    onOuiFormClone && onOuiFormClone.call(self, self, _$ouiControlDom, item["_index"], tdData["_index"]);
                                }
                                if (text.data) {
                                    _$ouiControlDom.attr("data", (typeof text.data === 'string') ? text.data : oui.parseString(text.data));
                                }
                                if (text.data4DB) {
                                    _$ouiControlDom.attr("data4DB", (typeof text.data4DB === 'string') ? text.data4DB : oui.parseString(text.data4DB));
                                }
                                if (typeof text.value !== 'undefined' && text.value !== null) {
                                    _$ouiControlDom.attr("value", oui.escapeHTMLToString(text.value));
                                }
                                tdData["ouiControlDomHTML"] = _$ouiControlDom[0].outerHTML;
                                tdData["text"] = $.trim(text.value).length > 0 ? text : "&nbsp;";
                            }
                        }
                        ...
                        if (showTitle + '' === 'true') {
                            ...
                            if (columnTitleRenderFunc) {
                                if (typeof columnTitleRenderFunc === 'function') {
                                    ...
                                    try {
                                        ...
                                        _title = f(title, item, item["_index"], columnField, tableOuiId);
                                    } catch (e) {
                            ...
                            title = ((isEscape + "" === "true") ? oui.escapeStringToHTML(title, false) : title);
                            if (title + '' !== 'null') {
        ...
        for (var j = 0, jlen = columnFields.length; j < jlen; j++) {
            ...
            if (!(columnField['columns'] && columnField['columns'].length > 0)) {
                ...
                if (columnField.columnType && columnField.columnType.length > 0) {//如果是特殊列
                    ...
                    var fieldName = columnField["summaryField"] || columnField["fieldName"] || "";
                    ...
                    if (summaryType && summaryType.length > 0) {
                        ...
                        if (!$ouiControlDom) {
                            ...
                            tdData["text"] = text.substring(0, text.lastIndexOf("<br/>")) || "&nbsp;";
    ...
    var formControlUpdateAfter = function (control) {
        ...
        if (control.attr("type") === 'uploadfile' || control.attr("type") === 'lbs') {
    ...
    var formControlUpdate = function (control, isEdited, isBatchEdit) {
        ...
        var i, len;
        if (!isBatchEdit) {
            rowIndex = controlId.substring(controlId.indexOf("rows") + 4, controlId.indexOf("cell"));
        } else {
        ...
        for (i = 0, len = tableData.length; i < len; i++) {
            rowData = tableData[i];
            if ((isBatchEdit && (rowIds.length <= 0 || rowIds.indexOf(rowData["_index"]) > -1)) || (rowData["_index"] + '' === rowIndex + '')) {
                ...
                if ((fileData && ('object' === typeof fileData))) {
                    ...
                    fileData["data"] = (typeof controlData === 'string') ? controlData : oui.parseString(controlData);
                    ...
                    fileData = {
                        ...
                        value: controlValue,
                        data: (typeof controlData === 'string') ? controlData : oui.parseString(controlData),
    ...
    var formControlUpdate4SingleEdit = function (control) {
        ...
        var rowIndex = controlId.substring(controlId.indexOf("rows") + 4, controlId.indexOf("cell"));
    ...
    var afterRender = function () {
        ...
        //解析列配置
        if ($.trim(configsHTML).length > 0) {//使用html方式进行配置
    ...
    var initStyle = function () {
        ...
        if (styleHtml && styleHtml.length > 0) {
            ...
            if ($style.length <= 0) {
                self._$tableGridView.prepend('<style type="text/css">' + styleHtml + '</style>');
            }
        }
    };
        ...
        if (ouiPagerDom) {
            ...
            self._pagerControl.attr("onGo2pageOk", function (pager, result) {
                if (oui.os.mobile) {
                    ...
                    if (pager.attr("pageIndex") !== 0 && pager.attr("pageIndex") !== 1) {
                    ...
                    self._$columns.find("#table_" + self.attr("ouiId") + "_all_checkbox").prop("checked", false);
                    self.renderByPager(pager, result);
                    self.scrollIntoView(0);
                }
                self._pagerControl.oldGoPageOK && self._pagerControl.oldGoPageOK(pager, result);
            });
            if (oui.os.mobile) {
                self._pagerControl.attr("onScroll2Top", function (/*pager*/) {
                    self.scrollIntoView(0);
                });
            }
        } else {
    ...
    var initView = function () {
        ...
        self._$columnsChooseOperateBtns = self._$columnChooseList.find(".column-screen-list-btns");
        ...
        self._$summaryRowLock = self._$summaryRow.find(".oui-grid-summaryRow-lock");
        self._$summaryRowView = self._$summaryRow.find(".oui-grid-summaryRow-view");
        ...
        self._getTableEditView = function (isCreate) {
            ...
            if ($editView.length <= 0 && isCreate) {
                self._$viewport.append("<div id='ouiTableEditView' style='position: absolute;display: none;z-index: 99;'></div>");
            }
    ...
    var getSummaryColumn = function (columnsArray) {
        var flag = false;
        for (var i = 0, len = columnsArray.length; i < len; i++) {
            if (columnsArray[i]["summaryType"] && columnsArray[i]["summaryType"].length > 0) {
                flag = true;
                break;
            }
        }
    ...
    var dataPutIdAndIndex = function (data) {
        for (var i = 0, len = data.length; i < len; i++) {
            data[i]["_uuid"] = data[i]["_index"] = data[i]["_id"] = getRowsUUid.call(this);
        }
    };
    ...
    initRenderFunc = function () {
        ...
        self._R_summaryRow = template.compile(TableGrid.tableSummaryRowsTpl[showType]);
        ...
        self._RGridWidth = function () {
            ...
            var rowMarginLeft = 0;
            if (lockColumnSize > 0 && lockColumnSize < columnsArray.length && noLockColumnsArray.length > 0) {
                ...
                _self._$rowsView.css("width", _self._$columns.width() - headerColumnsViewWidth);
            ...
            if (allWidth < _s) {
                ...
                for (i = 0, len = columnsArray.length; i < len; i++) {
                    ...
                    if (
                        ...
                        columnsArray[i]["width"] = (_s * (columnsArray[i]["width"] / allWidth));
                    }
                }
            }
        ...
        self._RRowsHeight = function () {
            ...
            if (tableGird.attr("style") && tableGird.attr("style").indexOf("height") > -1) {
                var maxHeight = tableGird.css("max-height");
                var viewHeight = self._$viewport.height();
                maxHeight = Number(maxHeight ? (maxHeight + "").replace("px", "") : 0) || 0;
                if (maxHeight) {
                    ...
                    if (maxHeight <= viewHeight || viewContent >= maxHeight) {
                        ...
                        offsetHeight = maxHeight - self._$columns.outerHeight() - self._$summaryRow.outerHeight() - self._$gridFooter.outerHeight();
                    ...
                    _self._$summaryRow.addClass("oui-grid-summaryRow-fixed");
                    var offsetHeight = tableGird.height() - self._$columns.outerHeight() - self._$summaryRow.outerHeight() - self._$gridFooter.outerHeight();
                    self._$rows.height(offsetHeight);
                }
            } else {
                self._$rows.height("auto");
        ...
        self._RColumnsView = function (notCalc) {
            ...
            if (lockColumnSize > 0 && lockColumnSize < columnsArray.length && noLockColumnsArray.length > 0) {
                ...
                parseHeaderRows.call(_self, noLockColumnHeaderData, noLockColumnsArray);
                ...
                _self._$columnsLock.find(rowsContentSelector).empty().html(lockColumnHtml);
                ...
                _self._$columnsView.find(rowsContentSelector).empty().html(noLockColumnHtml);
                setTimeout(function () {
                    _self._$rowsView.css("width", _self._$columns.width() - headerColumnsViewWidth);
                    _self._$columnsView.css({
                        "margin-left": headerColumnsViewWidth,
        ...
        self._RRowsViewByData = function (data, noAppend) {
            ...
            if (lockColumnSize > 0 && lockColumnSize < columnsArray.length && noLockColumnsArray.length > 0) {
                ...
                bodyData = serverData2tableData.call(self, data, lockColumnsArray, oldDataLength);
                ...
                bodyData = serverData2tableData.call(self, data, noLockColumnsArray, oldDataLength);
                var nolockHtml = self._R_RowsTable({
                    tableAttr: _self.getMap(),
                ...
                if (noAppend + '' === 'true') {
                    ...
                    _self._$rowsViewContent.find(rowsContentSelector).append(nolockHtml);
                }
            } else {
                _self._$rowsLock.hide();
        ...
        self._RRowsViewByIndex = function (data, renderType, index) {
            ...
            if (renderType === 0) {
                for (i = 0, len = data.length; i < len; i++) {
                    oldData.splice((index || 0) + i, 0, $.extend(true, {}, rowObjectFilter(data[i])));
                }
                ...
                for (i = 0, len = data.length; i < len; i++) {
                    ...
                    if (index >= 0) {
                        ...
                        var $lockContentView = _self._$rowsLockContent.find("#" + tableTrID);
                        var $rowsContentView = _self._$rowsViewContent.find("#" + tableTrID);
            ...
            if (renderType === 0) {
                ...
                if (lockColumnSize > 0 && lockColumnSize < columnsArray.length && noLockColumnsArray.length > 0) {
                    ...
                    $indexTr = _self._$rowsLockContent.find("#table_" + (_self.attr("ouiId")) + "_tr_" + row4IndexData["_index"]);

                    if ($indexTr.length <= 0) {
                        _self._$rowsLockContent.find(rowDomSelector + ":eq(0)").after(lockHtml);
                    } else {
                    ...
                    $indexTr = _self._$rowsViewContent.find("#table_" + (_self.attr("ouiId")) + "_tr_" + row4IndexData["_index"]);

                    rowMarginLeft = headerColumnsViewWidth;

                    if ($indexTr.length <= 0) {
                        _self._$rowsViewContent.find(rowDomSelector + ":eq(0)").after(nolockHtml);
                    } else {
                        $indexTr.after(nolockHtml);
                    }
                    _self._$rowsViewContent.find("#table_" + (_self.attr("ouiId")) + "_tr_noData").remove();
                } else {
                    _self._$rowsLock.hide();
                    ...
                    $indexTr = _self._$rowsViewContent.find("#table_" + (_self.attr("ouiId")) + "_tr_" + row4IndexData["_index"]);
                    if ($indexTr.length <= 0) {
                        _self._$rowsViewContent.find(rowDomSelector + ":eq(0)").after(html);
                    } else {
                        $indexTr.after(html);
                    }
                    _self._$rowsViewContent.find("#table_" + (_self.attr("ouiId")) + "_tr_noData").remove();
                ...
                if (lockColumnSize > 0 && lockColumnSize < columnsArray.length && noLockColumnsArray.length > 0) {
                    ...
                    $indexTr = _self._$rowsLockContent.find("#table_" + (_self.attr("ouiId")) + "_tr_" + index);
                    ...
                    $indexTr = _self._$rowsViewContent.find("#table_" + (_self.attr("ouiId")) + "_tr_" + index);
                    oui.clearByContainer($indexTr);
                ...
                self._$rows.find(oui.$.constant.controlTagName).each(function () {
                    ...
                    if (self.isCard) {
                        ...
                        if (_$tr.length > 0 && _$tr.hasClass('row-edit-layout') && (right === 'edit4ReadOnly')) {
        ...
        self._RSummaryRow = function () {
            ...
            if (lockColumnSize > 0 && lockColumnSize < columnsArray.length && noLockColumnsArray.length > 0) {
                ...
                _self._$summaryRowLock.find(rowsContentSelector).empty().html(lockColumnHtml);
                ...
                _self._$summaryRowView.find(rowsContentSelector).empty().html(noLockColumnHtml);
                setTimeout(function () {
                    _self._$summaryRowView.css({
                        "margin-left": headerColumnsViewWidth,
                        width: _self._$summaryRow.width() - headerColumnsViewWidth
                ...
                bodyData = getSummaryTableData.call(self, data, _self.attr("columnsArray"));
    ...
    var initEvents = function () {
        ...
        self._$columns.find(".oui-grid-table").on("mousedown", ".column-splitter", function (e) {
            ...
            self._$tableGridView.on("mousemove", function (e) {
                $proxyColumn.width((e.clientX - startX + startWidth) < 50 ? 50 : (e.clientX - startX + startWidth));
            ...
            self._$tableGridView.on("mouseup", function () {
                ...
                $proxyColumn.remove();
                var scrollerWidth = self._$rowsViewContent.find(".oui-grid-table").width() - self._$rowsViewContent.width();
                ...
                if (nextColumn && offsetWidth < 0 && scrollerWidth <= 0) {
                    ...
                    self._$columns.find("td#" + nextColumn["_id"]).css("width", nextColumn["width"]);
                    self._$rows.find("td#" + nextColumn["_id"]).css("width", nextColumn["width"]);
                    self._$summaryRow.find("td#" + nextColumn["_id"]).css("width", nextColumn["width"]);
                }
        ...
        self._$columns.find(".oui-grid-table").on("mouseover", "td", function () {
            ...
            if (showColumnsMenu + '' === 'true') {
                ...
                if (_column) {
                    ...
                    if ((columnType !== SpecialColumns.INDEX_COLUMN.type &&
                        ...
                        var canShowColumns4Menu = (showColumnsMenu + '' === 'true') && (_column['hideable4menu'] + '' === 'false');
                        if (canShowColumns4Menu) {
                            self._$showOrHideColumnBtns.css("display", "inline-block");
                            var position = $td.position();
                            var left = $td.offset().left - self._$columns.offset().left + $td.width() - self._$columnsOperateBtns.width();
        ...
        self._$showOrHideColumnBtns.on("click", function () {
            ...
            if (showColumnsMenu + '' === 'true') {
                ...
                for (var i = 0, len = columnsArray.length; i < len; i++) {
                    ...
                    var columnType = _column['columnType'];
                    // 这里判断了下是否是父列，如果是父列不支持显示,如果是特殊列也不显示
                    if (_column['hideable4menu'] + '' === 'false' &&
                ...
                var columnChooseListHtml = self._R_columnChooseList({columnList: show4menuArray});

                self._$columnChooseList.find("ul").html(columnChooseListHtml);

                self._$columnChooseList.css({left: postion.left - 5, top: postion.top + $btns.height()}).show();
            }
        });

        self._$columnsChooseOperateBtns.find(".btn-ok").on("click", function () {
            var $inputCheckboxs = self._$columnChooseList.find("ul").find("input[type='checkbox']");
        ...
        self._$columnsChooseOperateBtns.find(".btn-cancel").on("click", function () {
            self._$columnChooseList.hide();
        });

        self._$columns.find(".oui-grid-table").on("click", ".column-batchEdit", function (e) {
            var editModel = self.attr("editModel");
        ...
        self._$columns.find(".oui-grid-table").on("click", "td", function () {
            ...
            if (column["allowSort"] + '' === 'true' && column["fieldName"] && column["fieldName"].length > 0) {
                var sortOrder = ORDER_TYPE.ASC;
                if ($td.hasClass("grid-sort-desc")) {
                    self._$columns.find(".grid-sort-desc").removeClass("grid-sort-desc");
        ...
        //全选复选框的改变事件
        self._$columns.on("change", "#table_" + ouiId + "_all_checkbox", function () {
            var multiSelect = self.attr("multiSelect");
            var $inputCheckbox = $(this);
            if (multiSelect + '' === 'true') {
                if ($inputCheckbox.prop("checked")) {
                    self._$rows.find("input[name='table_" + ouiId + "_checkbox']").prop("checked", true);
                    self._$rows.find(rowSelector).not("#table_" + ouiId + "_tr_noData").addClass("selected");
                    triggerAllDataRowSelected.call(self, true);
                } else {
                    self._$rows.find("input[name='table_" + ouiId + "_checkbox']").prop("checked", false);
         ...
         */
        self._$rows.on("change", "input[name='table_" + ouiId + "_checkbox']", function () {
            ...
            if ($inputCheckbox.prop("checked")) {
                if (multiSelect + '' !== 'true') {
                    self._$rows.find("input[name='table_" + ouiId + "_checkbox']:checked").prop("checked", false);
                    self._$rows.find(rowSelector + ".selected").removeClass("selected");
                    triggerAllDataRowSelected.call(self, false);
                }
                row._selected = true;
                $inputCheckbox.prop("checked", true);
                if (isAllSelected.call(self)) {
                    self._$columns.find("#table_" + ouiId + "_all_checkbox").prop("checked", true);
                }
                ...
                self._$columns.find("#table_" + ouiId + "_all_checkbox").prop("checked", false);
                row._selected = false;
                $inputCheckbox.prop("checked", false);
        ...
        self._$rows.on("click", rowClickSelector, function (e) {
            ...
            if (column) {
                ...
                if (columnClick) {
                    ...
                    columnClick && columnClick.call(self, self, row, cValue, column, $td, $tr);
                    hasCellClick = true;
                } else {
                    if (self.attr("onCellclick")) {
                        hasCellClick = true;
                    }
                    triggerEvent.call(self, "onCellclick", self, row, cValue, column, $td, $tr);
                }
            }
            //FIXME 移动端如果是复选框TD则不触发rowClick
            if (column.columnType !== SpecialColumns.CHECKBOX_COLUMN.type && !hasCellClick) {
                triggerEvent.call(self, "onRowclick", self, row, $tr, column);
            }
            var editModel = self.attr("editModel");
            if (editModel === 'click') {//如果是双击编辑模式
        ...
        self._$rows.on("dblclick", rowSelector + " td", function (e) {
            ...
            if (column) {
                triggerEvent.call(self, "onCelldblclick", self, row, oui.JsonPathUtil.getJsonByPath(column["fieldName"], row), column, $td);
            }
            triggerEvent.call(self, "onRowdblclick", self, row, $tr);
            var editModel = self.attr("editModel");
            if (editModel === 'dblClick') {//如果是双击编辑模式
                createOuiForm4edit.call(this, column, row, $td);
            }
        });
        ...
        var createOuiForm4edit = function (column, row, $td, isBatchEdit) {
            ...
            if (!isBatchEdit) {
                formControlUpdateAfter = "oui.getByOuiId(" + self.attr("ouiId") + ").formControlUpdate4SingleEdit";
                text = oui.JsonPathUtil.getJsonByPath(column['fieldName'], row); //row[column['fieldName']];
                ouiFormId = self.attr("id") + "rows" + row["_index"] + "cell" + column["_index"] + "_control";
                var onAfterUpdateStr = _$ouiControlDom.attr("onafterupdate");
                if (onAfterUpdateStr && onAfterUpdateStr.length > 0) {
                    onAfterUpdateStr += "," + formControlUpdateAfter;
                } else {
                ...
                ouiFormId = self.attr("id") + "batchEditCell" + column["_index"] + "_control";
                top += $td.outerHeight();
            }
            ...
            if (isBatchEdit) {
                ...
                //确定按钮
                $rowEditView.find(".batchEdit-btn-submit").off("click").on("click", function () {
                    var control = oui.getById(ouiFormId);
                    self.formControlUpdate4BatchEdit(control);
                    $rowEditView.html("").hide();
                });
                //取消按钮
                $rowEditView.find(".batchEdit-btn-cancel").off("click").on("click", function () {
                    //取消不显示
                    $rowEditView.html("").hide();
                });
            } else {
    ...
    var triggerRowSelectedByRowIndex = function (rowIndex, fireEvent, flag) {
        ...
        var ouiId = self.attr('ouiId');
        //以下处理 行的选中和 取消选中
        var $tr = self._$rowsViewContent.find("#table_" + ouiId + "_tr_" + rowIndex); //.closest("tr");
        var id = $tr.attr("id");
        var index = id.replace("table_" + ouiId + "_tr_", "");
        if (index + "" === "noData") return;
        index = $tr.index() - 1;
        ...
        if (!!flag) {
            ...
            row._selected = true;
            self._$rowsLock.find("#" + id).addClass("selected").find("input[name='table_" + ouiId + "_checkbox']").prop("checked", true);
            self._$rowsView.find("#" + id).addClass("selected").find("input[name='table_" + ouiId + "_checkbox']").prop("checked", true);
            if (isAllSelected.call(self)) {
                self._$columns.find("#table_" + ouiId + "_all_checkbox").prop("checked", true);
            }
            if (fireEvent) {
            ...
            row._selected = false;
            self._$rowsLock.find("#" + id).removeClass("selected").find("input[name='table_" + ouiId + "_checkbox']").prop("checked", false);
            self._$rowsView.find("#" + id).removeClass("selected").find("input[name='table_" + ouiId + "_checkbox']").prop("checked", false);
            self._$columns.find("#table_" + ouiId + "_all_checkbox").prop("checked", false);
            if (fireEvent) {
                triggerEvent.call(self, "onRowUnSelect", self, row, $tr);
            }
    ...
    var triggerAllDataRowSelected = function (flag) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            if (flag) {
                ...
                if ($tr.find("input[name='table_" + ouiId + "_checkbox']").length > 0) {
    ...
    var tableGetData = function (param, success, error) {
        ...
        if (dataURL && dataURL.length > 0) {
            oui.getData(dataURL, param, function (result) {
                result = oui.parseJson(result);
                if (result.success + '' === 'true') {
                    result = result['msg'];
                    result = oui.parseJson(result);
                    success && success(oui.JsonPathUtil.getJsonByPath(self.getDataFieldName(), result));
                } else {
    ...
    var initData = function (fromDataUrl, isTbodyLoad) {
        ...
        if (!isTbodyLoad && (!data || data.length <= 0 || fromDataUrl)) {
            if (self._pagerControl) {
                ...
                tableGetData.call(self, {}, function (data) {
                    data = oui.parseJson(data);
                    self.setData(data);
                    self._RRowsView();
                    self.scrollIntoView(0);
    ...
    var filterData = function (data, queryParam) {
        ...
        for (var i = 0, len = queryParam.length; i < len; i++) {
            ...
            var trueData = oui.JsonPathUtil.getJsonByPath(_paramCondition['field'], data);
        ...
        if (tableGird.attr("style") && tableGird.attr("style").indexOf("height") > -1) {
            ...
            if (maxHeight) {
                if (maxHeight <= viewHeight) {
                    _self._$summaryRow.addClass("oui-grid-summaryRow-fixed");
                    offsetHeight = maxHeight - _self._$columns.outerHeight() - _self._$summaryRow.outerHeight() - _self._$gridFooter.outerHeight();
                ...
                _self._$summaryRow.addClass("oui-grid-summaryRow-fixed");
                var offsetHeight = tableGird.height() - _self._$columns.outerHeight() - _self._$summaryRow.outerHeight() - _self._$gridFooter.outerHeight();
                _self._$rows.height(offsetHeight);
            }
        } else {
            _self._$rows.height("auto");
    ...
    var noDataTr = '{{if emptyTips && emptyTips.length > 0}}' +
        '<tr id="table_{{ouiId}}_tr_noData" class="oui-table-grid-row">' +
        '<td style="width: 0;border:0;"></td>' +
        '<td colspan="{{columnsArray.length}}" class="text-center grid-emptyText {{emptyTipsIcon+\'\' === \'true\' ? \'\':\'grid-emptyText-noIcon\'}}">' +
        '<div class="cell-inner ">{{=emptyTips}}</div>' +
        ...
        if (oldData && oldData.length > 0) {
            for (var i = 0, len = oldData.length; i < len; i++) {
                ...
                if (filterData(_d, queryParam)) {
                    ...
                    self._$rowsLockContent.find("#table_" + self.attr("ouiId") + "_tr_" + _d["_index"]).removeClass("grid-row-alt").addClass(cls).show();
                    self._$rowsViewContent.find("#table_" + self.attr("ouiId") + "_tr_" + _d["_index"]).removeClass("grid-row-alt").addClass(cls).show();
                } else {
                    self._$rowsLockContent.find("#table_" + self.attr("ouiId") + "_tr_" + _d["_index"]).hide();
                    self._$rowsViewContent.find("#table_" + self.attr("ouiId") + "_tr_" + _d["_index"]).hide();
                }
            }
        }

        if (data.length <= 0) {
            if (self._$tableGridView.find("#table_" + ouiId + "_tr_noData").length <= 0) {
    ...
    var updateRow = function (row, rowData, noShowEdit) {
        ...
        if (popupDialogOperateRow + '' === 'true' && self.attr("showType") === '1' && !noShowEdit) {
    ...
    var addRow = function (row, index, notShowEdit) {
        ...
        if (row) {
            ...
            var popupDialogOperateRow = self.attr("popupDialogOperateRow");
            if (popupDialogOperateRow + '' === 'true' && self.attr("showType") === '1' && !notShowEdit) {
                var newRow = newData[0];//添加行直接获取第一条
    ...
    var editAction = function (actionIndex) {
        ...
        // 外部逻辑
        if (actionIndex === 0) {//确定
            var flag = triggerEvent.call(self, "onPopupDialogOk", self, isUpdate, row);
            if (flag === false) {
                return false;
            }
        }

        self._$tableGridView.find(".rowEdit-buttons-content").removeClass("show-rowEdit-buttons");
        ...
        if (row) {
            ...
            if (_index > -1 && _index + 1 < data.length) {
                ...
                if (tempRow) {
                    var ouiId = self.attr("ouiId");
                    var d = null;
                    var tempLockTr = self._$rowsLockContent.find("#table_" + ouiId + "_tr_" + tempRow["_index"]);
                    var tempRowTr = self._$rowsViewContent.find("#table_" + ouiId + "_tr_" + tempRow["_index"]);
                    var currLoctTr = self._$rowsLockContent.find("#table_" + ouiId + "_tr_" + row["_index"]);
                    var currRowTr = self._$rowsViewContent.find("#table_" + ouiId + "_tr_" + row["_index"]);
                    if (index > _index) {
    ...
    var moveUp = function (row) {
        ...
        if (row) {
            ...
            if (index > 0) {
                ...
                var ouiId = self.attr("ouiId");
                var oldLockTr = self._$rowsLockContent.find("#table_" + ouiId + "_tr_" + rowIndex);
                var oldRowTr = self._$rowsViewContent.find("#table_" + ouiId + "_tr_" + rowIndex);
    ...
    var selectAll = function () {
        ...
        var ouiId = self.attr("ouiId");
        self._$columns.find("#table_" + ouiId + "_all_checkbox").prop("checked", true);
        self._$rowsLockContent.find("input[name='table_" + ouiId + "_checkbox']").prop("checked", true);
        self._$rowsLockContent.find(rowSelector).addClass("selected");
        self._$rowsViewContent.find("input[name='table_" + ouiId + "_checkbox']").prop("checked", true);
    ...
    var deselectAll = function () {
        ...
        self._$rowsLockContent.find("input[name='table_" + ouiId + "_checkbox']").prop("checked", true);
        self._$rowsLockContent.find(rowSelector).addClass("selected");
        self._$rowsViewContent.find("input[name='table_" + ouiId + "_checkbox']").prop("checked", false);
    ...
    var showOrHideColumn = function (param, visible) {
        ...
        if (typeof param !== "undefined") {
            ...
            if (typeof param === 'number') {
                ...
                setColumnsByObject.call(self, {fieldName: param}, {visible: !!visible});
                self._RColumnsView(true);
                self._RRowsView();
            }
        }
    ...
    var showOrHideColumns = function (fieldNames, visible) {
        ...
        if (fieldNames && fieldNames.length > 0) {
            ...
            for (var i = 0, len = fieldNames.length; i < len; i++) {
                ...
                setColumnsByObject.call(self, {fieldName: fieldName}, {visible: !!visible});
            }
            self._RColumnsView(true);
            self._RRowsView();
        }
    ...
    var scrollIntoView = function (rowIndex) {
        ...
        if (typeof rowIndex !== 'undefined') {
            ...
            if ($tableGrid.attr("style") && $tableGrid.attr("style").indexOf("height") > -1) {
    ...
    var findControlByRowAndfieldName = function (rowData, fieldName) {
        ...
        var ouiFormId = this.attr("id") + "rows" + rowIndx + "cell" + cellIndex + "_control";
    ...
    var addColumn = function (column, index) {
        ...
        var columnsMap = {};
        var editColumnsArray = [];
        _parseColumnsPutArrayAndMap(columnsArray, columnsFieldsArray, columnsMap, columns, editColumnsArray);
        self.attr("maxHeaderLevel", level);
        self.attr("columns", columns);
        self.attr("columnsFieldsArray", columnsFieldsArray);
        self.attr("columnsArray", columnsArray);
    ...
    var setColumnContextMenu = function (config) {
        ...
        if (config) {
            ...
            if (count > 0) {
                ...
                var $trTwo = self._$columnsView.find(".oui-grid-table tr").eq(1);
                $trTwo.find(".header-cell-outer").contextMenu("ouiTableColumnContextMenu", {
                    bindings: bindings
                });
            }
        }
    ...
    var toggleNumColumn = function () {
        ...
        if (showNumColumn + "" === "true") {
            if (!indexColumn) {
                addColumn.call(self, '<oui-column-index width="70" >序号</oui-column-index>', -1);
            }
        } else {
            if (indexColumn) {
    ...
    function formatNumber(format, text, dotNum) {
        ...
        if (formatType && formatType.length > 0 && (text !== null && typeof text !== 'object' && typeof text !== 'undefined' && text !== '')) {
            if (!isNaN(text)) {
                text = Number(text + "");
                if (formatType === ',') {//千分位
            ...
            if (dotNum !== null && typeof dotNum !== 'undefined' && dotNum !== '' && dotNum + "" !== '-1' && (text !== null && typeof text !== 'object' && typeof text !== 'undefined' && text !== '')) {
