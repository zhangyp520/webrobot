(function($) {
  ...
  Dragging.prototype.start = function(parent, el, e, handle) {
    ...
    // the pointer events fire on the elements underneath the webroboter
...
//TODO 添加方向固定属性
  ...
  Draggable.prototype.start = function(e) {
    ...
    var el = this.el, webroboter
    if (this.opts.clone) {
      if (typeof this.opts.clone === 'function') {
        webroboter = this.opts.clone.call(this.el)
      } else {
        webroboter = this.el.clone()
        if (this.opts.cloneClass) {
          webroboter.addClass(this.opts.cloneClass)
        }
      }
      var position = this.el.position()
      webroboter.css('position', 'absolute')
          .css('left', position.left).css('top', position.top)
          .width(this.el.width()).height(this.el.height())
      webroboter.insertAfter(this.el)
    }

    dragging.start(this, this.el, e, webroboter)
  ...
  Droppable.prototype.drop = function(e) {
    ...
    var clone = null
    if (dragging.handle) {
      evtObj.webroboter = dragging.handle
      if (!handler) {
