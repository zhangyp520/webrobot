!(function(win){
    ...
    oui.bindDraggable = function(cfg){
        ...
        win.$(dragSelector,container).draggable({
            ...
            cancel: "[canuse=false]",
            webroboter: "clone", // webroboter可以指定为“original”, "clone"，其中"original"是默认值，即拖动自身；而clone表示创建自身的一个克隆用于拖拽。
        ...
        obj.bindSort = function(){
            ...
            oui.bindSortable({
                ...
                receive:function(event,ui){
                    var data = currDialog.attr('data') || [];
                    var selects = currDialog.attr('selects')||[];
                    var id = ui.webroboter.attr('drag-id');
                    ...
                    ui.webroboter.removeAttr('drag-id');
                    ui.webroboter.attr('sort-id',id);
