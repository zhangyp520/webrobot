!(function(win){
    ...
    oui.componentAdapter = {
        ...
        'vue':{
             ...
             * @param el
             */
            render4include:function(compClazz,options,success,error,el){
             ...
             * @param error
             */
            render:function(compClazz,options,success,error){
                try{
                    //先销毁再创建
     ...
     * @param options
     */
    oui.getComponent = function(url,options,isInclude){
        /**资源不存在则执行 缓存 **/
        ...
        if(oui.componentAdapter[templateType].createInstance){
            return oui.componentAdapter[templateType].createInstance(compClazz,options,isInclude);
        }else{
     ...
     * @param error
     */
    oui.renderComponent = function(url,options,success,error,el){
        ...
        if(el){ // include
            if(oui.componentAdapter[templateType].render4include){
                oui.componentAdapter[templateType].render4include(compClazz,options,success,error,el);
            }else{
            ...
            if(oui.componentAdapter[templateType].render){
                oui.componentAdapter[templateType].render(compClazz,options,success,error);
            }else{
