(function(win){
	 ...
	 */
	Dialog.templateHtml=[];
	Dialog.templateHtml[0] ='<div class="oui-dialog oui-show">'+
	...
	Dialog.templateHtml[1] ='<div class="oui-dialog oui-show">'+
	...
	'</div>'+
	'</div>' ;
	Dialog.templateHtml[2]='<div class="oui-loading-block oui-dialog oui-show">'+
	...
	'</div>';
	Dialog.templateHtml[4] ='<div class="oui-dialog oui-show">'+
	'<div class="oui-dialog-cnt oui-animated  oui-zoomIn ">'+
	'<header class="oui-dialog-hd oui-border-b">'+
	'<h3>{{=title}}</h3>'+
	'<i class="oui-dialog-close" onclick="oui.getByOuiId({{ouiId}}).closeClick(\'close\')" targetOuiId={{ouiId}}></i>'+
	...
	'</div>'+
	'</div>' ;
	Dialog.templateHtml[5] ='<div class="oui-dialog oui-show">'+
	'<div class="oui-dialog-cnt">'+
	'<header class="oui-dialog-hd oui-border-b">'+
	'<h3>{{=title}}</h3>'+
	'<i class="oui-dialog-close" onclick="oui.getByOuiId({{ouiId}}).closeClick(\'close\')" targetOuiId={{ouiId}}></i>'+
	...
	'</div>' ;
	Dialog.templateHtml[6] ='<div class="oui-dialog oui-show">'+
	'<div class="oui-dialog-cnt">'+
	'<header class="oui-dialog-hd oui-border-b">'+
	'<h3>{{=title}}</h3>'+
	'<i class="oui-dialog-close" onclick="oui.getByOuiId({{ouiId}}).closeClick(\'close\')" targetOuiId={{ouiId}}></i>'+
	...
	'</div>'+
	'</div>' ;
	Dialog.templateHtml[7] ='<div class="oui-dialog oui-show">'+
	...
	Dialog.templateHtml[8]='<div class="oui-dialog oui-show" style="">'+
	 ...
	 * 进度条模板
	 */
	Dialog.templateHtml[9]='<div class="oui-dialog  oui-show">'+
		...
		'</div>'+
	'</div>' ;
	Dialog.templateHtml[10] ='<div class="oui-dialog oui-show">'+
	 ...
	 */
	Dialog.templateHtml[11]='<div class="oui-dialog oui-show">'+
		'<div class="oui-dialog-area" style="{{contentStyle}}">'+
			...
			'{{if url}}'+
			'<iframe src="{{url}}&ouiDialogId={{ouiId}}"></iframe>'+
			'{{/if}}'+
	...
	oui.openWindow = function(cfg){
		...
		for(var i in window._openMap){
			try{
				if(window._openMap[i]&&window._openMap[i].win&&window._openMap[i].win.closed){
		...
		Dialog.upDialog.attr({
			...
			*/
			completSuccess:function(){
				...
				options.completSuccess && options.completSuccess(result,swfUpload);
	...
	oui.open=function(option){
		...
		if(url){ //目前是iframe弹出框，不用 ajax弹出框
			option["showType"]=11;
			url = oui.addOuiParams4Url(url);
			if(url.indexOf('?')<0){
	...
	oui.showCodeDialog = function(json,callback,styleCfg){
		...
		oui.getTop().oui.showInputDialog(styleCfg&&styleCfg.title||'脚本编辑', function(result){
			...
			type:"textarea",value:bodyCode,readOnly:false,rows:15-startCodeRows-endCodeRows ,placeholder:" ",style:'line-height:20px;height:auto;',inputAreaStyle:'margin-bottom:0;padding: 0 10px;'//内容行数
		},{
     ...
     *      completSuccess:function(results){},//返回文件
     *
